...
def prob_next_nodes(graph, current_node, pagerank_vector, teleport):
    ...
    prob_next_nodes = []
    if teleport:
        prob_next_nodes = [pagerank_vector[i] if next_nodes_vector[i] != 0 else 0 for i in range(len(pagerank_vector))]
...
def power_iterate(graph, pagerank_vector, teleport, start_node,num_iterations):
    ...
    for _ in range(num_iterations):
        ...
        current_node=random_walk(graph,walked_nodes[-1],pagerank_vectors[-1],teleport)
